...
interface Column {
  key: string;
  label: string;
  composite?: string[]; // Para columnas que concatenan múltiples campos
}
...
interface RepertorioSearchProps {
  ...
  title: string;
  columns: Column[];
  searchFields: { id: string; label: string; type: string; options?: string[]; searchIn?: string[] }[];
    ...
    try {
      ...
      Object.entries(filters).forEach(([key, value]) => {
        ...
        if (key === 'fecha_desde') {
          ...
          // Check if this filter has searchIn (multi-field OR search)
          const field = searchFields.find(f => f.id === key);
          if (field?.searchIn && field.searchIn.length > 0) {
            // Build OR filter for multiple columns: col1.ilike.%val%,col2.ilike.%val%
                    ...
                    {columns.map((col) => (
                      <td key={col.key} className="px-6 py-4 text-sm text-slate-700">
                        {col.key === 'fecha'
                          ...
                          : col.composite
                            ? col.composite.map(f => row[f] || '').filter(Boolean).join(' ')
                            : (row[col.key] || '-')}
                      </td>
                    ))}
                  </tr>

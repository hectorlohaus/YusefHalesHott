...
const UI = {
    ...
    renderForm() {
        ...
        schema.formFields.forEach(field => {
            ...
            } else if (field.type === 'datalist' || field.type === 'select') {
                inputHtml = `<select id="form-${field.id}" ${requiredAttr} class="${baseClass}"><option value="">Seleccione...</option>${field.options.map(o => `<option value="${o}">${o}</option>`).join('')}</select>`;
            } else {

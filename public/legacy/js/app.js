...
const App = {
    ...
    async startEdit(id) {
        ...
        const { data: perfil } = await State.supabase.from('usuarios').select('rol').eq('auth_id', user?.id).single();
        if (!perfil || !['notario', 'supervisor'].includes(perfil.rol)) {
            UI.showError("No tienes permiso para editar registros. Sólo los Notarios/Supervisores pueden hacerlo.");
            return;
        }
    ...
    async saveRecord() {
        ...
        try {
            ...
            if (State.editingId) {
                ...
                const { data: perfil } = await State.supabase.from('usuarios').select('rol').eq('auth_id', user?.id).single();
                if (!perfil || !['notario', 'supervisor'].includes(perfil.rol)) {
                    throw new Error("Permiso denegado para editar. Sólo los Notarios/Supervisores pueden hacerlo.");
                }
                ...
                if (deleteBtn) {
                    ...
                    newConfirm.addEventListener('click', async () => {
                        ...
                        try {
                            ...
                            const { data: perfil } = await State.supabase.from('usuarios').select('rol').eq('auth_id', user?.id).single();
                            if (!perfil || !['notario', 'supervisor'].includes(perfil.rol)) {
                                UI.showError('No tienes permiso para eliminar registros. Sólo los Notarios/Supervisores pueden hacerlo.');
                                return;
                            }
